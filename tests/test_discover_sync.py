...
def test_discover_devices(name, rsp_slaves, rsp_dict, exp_devices, request):
    ...
    def on_read(api: NextApiStub, address: int, count: int, slave: int):
        ...
        if slave in rsp_slaves and str(address) in rsp_dict:
            ...
            registers = ModbusTcpClient.convert_to_registers(value=value, data_type=data_type)
    ...
    devices = discover.discover_devices(getExtendedInfo=False)
...
def test_discover_extendedinfo(name, rsp_slaves, rsp_dict, exp_devices, exp_serial, exp_sw_version, exp_om_version, request):
    ...
    discover = NextDiscover(api, dataset)   
    devices = discover.discover_devices(getExtendedInfo=True)
...
def test_gateway_info(name, rsp_slaves, rsp_dict, exp_host, exp_guid, request):
    ...
    # Perform the discover
    gateway_info = discover.discover_gateway_info()
