...
class AsyncXcomDiscover:
        ...
        for family in NextDeviceFamilies.get_list():
            ...
            for device_slave in range(family.slave_devices_start, family.slave_devices_end+1):
                ...
                try:
                    ...
                    value = await self._api.request_value(param, device_slave, verbose=verbose)
    ...
    async def _request_value_by_name(self, param_name, family_id, device_slave, verbose=False):
        try:
            param = self._dataset.get_by_name(param_name, family_id)
            return await self._api.request_value(param, device_slave, verbose=verbose)
        except:
